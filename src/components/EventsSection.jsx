  ...
  return (
    <section className="py-16 bg-gradient-to-br from-cream to-warm-gray">
      <div className="container mx-auto px-4">
          ...
          {upcomingEvents.map((event, index) => (
            <div key={index} className="bg-cream-light/80 backdrop-blur-sm rounded-lg p-6 hover:bg-cream-light/90 transition-all duration-300 border border-primary-gold/20 shadow-lg hover:shadow-xl">
              <div className="flex items-center justify-between mb-4">
                <FaCalendarAlt className="text-2xl text-primary-gold" />
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                  event.status === 'Booking Open' ? 'bg-green-100 text-green-700 border border-green-200' :
                  event.status === 'Early Bird' ? 'bg-gold/20 text-primary-gold border border-gold/30' :
        ...
        <div className="bg-warm-white/80 backdrop-blur-sm rounded-lg p-8 border border-primary-gold/20 shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {eventDestinations.map((destination, index) => (
              <div key={index} className="bg-cream-light/90 rounded-lg p-6 border border-primary-gold/15 shadow-md hover:shadow-lg transition-all duration-300 hover:border-primary-gold/30">
        ...
        <div className="text-center mt-12">
          ...
          <UTMLink 
            to="/booking"
            className="inline-flex items-center gap-2 bg-gradient-to-r from-gray-700 to-gray-800 hover:from-gray-600 hover:to-gray-700 text-primary-gold px-8 py-3 rounded-lg font-medium transition-all duration-300 transform hover:scale-105 shadow-lg border border-primary-gold/20 hover:border-primary-gold/40"
          >
            Reserve Now
          </UTMLink>
        </div>
